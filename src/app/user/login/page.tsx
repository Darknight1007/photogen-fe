...
export default function UserLogin() {
  ...
  const handleVerifyOtp = async (e: React.FormEvent) => {
    ...
    if (data?.user.role !== "USER") {
      setError("This account is registered as a photographer. Please use photographer login.");
      setLoading(false);
      return;
    }
  ...
  const handleSignup = async (e: React.FormEvent) => {
    ...
    const { data, error: apiError } = await authApi.userSignup({
      phone,
      otp,
      name: name.trim(),
      ...
      <div className="w-full max-w-md">
        ...
        <div className="card animate-fade-in">
          ...
          <div className="text-center mb-8">
            <div className="inline-flex items-center justify-center w-14 h-14 rounded-xl bg-gradient-to-br from-accent to-orange-600 mb-4">
              <svg className="w-7 h-7 text-white" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M16 7a4 4 0 11-8 0 4 4 0 018 0zM12 14a7 7 0 00-7 7h14a7 7 0 00-7-7z" />
              </svg>
            </div>
            <h1 className="text-2xl font-bold mb-2">
              {step === "signup" ? "Create Account" : "Find Your Photos"}
            </h1>
        ...
        <p className="text-center text-muted text-sm mt-6">
          ...
          <Link href="/photographer/login" className="text-primary hover:underline">
